...
port install openmpi tbb subversion git-core +svn git-extras gcc_select gmake autoconf automake libtool texlive-latex texlive-latex-extra texlive-latex-recommended bash bash-completion texlive-fonts-recommended texlive-fontutils texlive-bin-extra boost cgal eigen3 scons gsl doxygen graphviz gsed clang-3.1 clang_select glpk vtk5 

port install hdf5-18 +fortran +gcc45

# graphics utilities, useful with latex
port install ImageMagick inkscape
